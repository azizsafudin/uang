...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  // Keep password managers (1Password, LastPass, Dashlane, …) off ordinary
  // fields like "Name". Credential inputs still get autofill via their type.
  const isCredential = type === "email" || type === "password"
  const suppressManagers = isCredential
    ? {}
    : {
        autoComplete: "off",
        "data-1p-ignore": true,
